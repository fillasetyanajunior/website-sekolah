...
class AttandanceController extends Controller
{
    ...
    public function absenall(Request $request)
    {
        ...
        if ($request->kelas == 'X') {
            ...
            $student    = StudentDetail::where('kelas', $request->kelas)->where('jurusan', $request->jurusan)->get();
            // $schedule   = Schedule::where('kelas', $request->kelas)->where('matapelajaran', $request->matapelajaran)->where('jurusan', $request->jurusan)->first();
                $schedule   = Schedule::where('hari', $hari[date('N')])->where('kelas', $student->kelas)->where('matapelajaran', $request->matapelajaran)->where('jurusan', $request->jurusan)->first();
        }
